...
model.add(Dense(512, input_dim=2048,init='glorot_normal',W_regularizer=l2(0.001),activation='relu'))
...
#    print("Loading training batch")
    ...
    for iv in Norm_list_iter:
        ...
        words = f.read().split()
        feat_row1 = np.array([])
        num_feat = len(words) /2048   # Number of features to be loaded. In our case num_feat=32, as we divide the video into 32 segments.
        num_feat = int(num_feat)
        count = -1
        VideoFeatues = []
        for feat in range(0, num_feat):
...
AllClassPath=os.getcwd() + '/SampleVideos'
